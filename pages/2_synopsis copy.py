...
with main_tab1:
    ...
    with movie_tab2:
        st.subheader("🎯 영화 BERTopic 클러스터 분석")
        st.markdown("""
        **분석 방법:**
        - 영화 줄거리(overview)와 장르(genres)를 결합하여 Qwen3 임베딩 생성
        - BERTopic으로 토픽 모델링 후 AgglomerativeClustering으로 유사 토픽 통합
...
with main_tab2:
    ...
    with drama_tab2:
        st.subheader("🎯 드라마 BERTopic 클러스터 분석")
        st.markdown("""
        **분석 방법:**
        - 드라마 줄거리(overview)와 장르(genres)를 결합하여 Qwen3 임베딩 생성
        - BERTopic으로 토픽 모델링 후 AgglomerativeClustering으로 유사 토픽 통합
        - 흥행작: hit_score 상위 20% (n_groups=4) / 비흥행작: hit_score 하위 20% (n_groups=8)
        """)
        st.markdown("---")
